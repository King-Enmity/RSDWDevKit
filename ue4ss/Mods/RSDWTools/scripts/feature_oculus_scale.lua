...
local function tick_scale()
    ...
    if dx ~= nil and dy ~= nil then
        local changed = apply_scale_delta(dx, dy)
...
function M.reset()
    ...
    return false, "reset scale write failed"
end

function M.status()
    if not state.active then return true, "inactive" end
    local scale = state.current_scale or {}
    return true, string.format("active target=%s mode=%s source=%s mouse=%s driver=%s writes=%d actor_fail=%d root=%d path=%s dx=%.3f dy=%.3f x=%.3f y=%.3f z=%.3f bounds=%.3f..%.3f",
        tostring(state.name),
        tostring(state.mode),
...
function M.handle_modal_hotkey(key)
    ...
    if k == "F" or k == "LEFT_MOUSE_BUTTON" or k == "LEFT_MOUSE" then
        M.stop()
        return true
